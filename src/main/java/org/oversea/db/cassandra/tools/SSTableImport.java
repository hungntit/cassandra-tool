/**
 * This file base on SSTableImport of apache cassandra. I have fixed some bugs when convert from json to sstable.
 */
...
public class SSTableImport
{
    ...
    private static class JsonColumn<T>
    {
        ...
        public JsonColumn(T json, CFMetaData meta, boolean isSubColumn)
        {
            if (json instanceof List)
            {
                ...
                timestamp =  fields.get(2) instanceof Long ? (Long)fields.get(2) : (Integer)fields.get(2);
                kind = "";
                ...
                if (fields.size() > 3)
                {
                    ...
                    else
                    {
                        ...
                        {
                            timestampOfLastDelete = (long) ((Long) fields.get(4));
                        }
    ...
    private void addColumnsToCF(List<?> row, ByteBuffer superName, ColumnFamily cfamily)
    {
	    ...
	    for (Object c : row)
	        {
	            JsonColumn<List<?>> col = new JsonColumn<List<?>>((List<?>) c, cfm, (superName != null));
	            QueryPath path = new QueryPath(cfm.cfName, superName, col.getName());
	
	            if (col.isExpiring())
	            {
	            	cfamily.addColumn(superName, new ExpiringColumn(path.columnName, col.getValue(), col.timestamp, col.ttl, col.localExpirationTime));
	            }
	            else if (col.isCounter())
	            {
	                cfamily.addColumn(superName, new CounterColumn(col.getName(), col.getValue(), col.timestamp, col.timestampOfLastDelete));
	            }
	    ...
	    if(row.size() == 0 && cfamily.isSuper()){
       		cfamily.addColumn(new SuperColumn(superName, cfamily.getSubComparator()));
        }
    }
    ...
    private int importUnsorted(String jsonFile, ColumnFamily columnFamily, String ssTablePath, IPartitioner<?> partitioner) throws IOException
    {
        ...
        keyCountToImport = countLines(new File(jsonFile));
